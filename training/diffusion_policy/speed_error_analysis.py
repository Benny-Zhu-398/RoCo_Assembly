...
def main() -> None:
    ...
    # Grouped checkpoints (train.py --group) use a different model class --
    # no single `part`/`unet`, vision-only, skill head chosen per-sample by
    # group -- see grouped_model.py / evaluate_grouped.py. Route to the
    # matching builder/sampler instead of assuming the single-part shape.
    is_group = ckpt.get("group") is not None
    if is_group:
    ...
    for batch in loader:
        ...
        sample_fn = ddim_sample_grouped if is_group else ddim_sample
        action_n_pred = sample_fn(
        ...
        pred = unnormalize_action(action_n_pred.cpu().numpy(), norm_stats, args.part)
        gt = unnormalize_action(action_n_gt.cpu().numpy(), norm_stats, args.part)
