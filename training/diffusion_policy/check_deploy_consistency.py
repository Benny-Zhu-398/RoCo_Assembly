...
def eval_raw7(action_n: np.ndarray, norm_stats: NormStats, part: str) -> np.ndarray:
    """evaluate.py's own bound unnormalize_action."""
    return evaluate_mod.unnormalize_action(
        np.asarray(action_n, dtype=np.float32).reshape(1, -1), norm_stats, part
    )[0]
...
def adapter_raw7_and_command(action_n: np.ndarray, norm_stats: NormStats, part: str):
...
def run_action_check(part: str, frames, norm_stats: NormStats, right_const: np.ndarray,
    ...
    for ep, t, _state44, action14 in frames:
        ...
        action_n = dataset.normalize_action(raw_left, norm_stats, part)
    ...
    for label, action_n in test_vectors:
        raw7_eval = eval_raw7(action_n, norm_stats, part)
        raw7_adapter, pos, quat, grip, used_fallback = adapter_raw7_and_command(action_n, norm_stats, part)
        n_checked += 1
